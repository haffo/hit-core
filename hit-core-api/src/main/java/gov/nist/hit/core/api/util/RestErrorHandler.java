...
public class RestErrorHandler {
  ...
  public ResponseMessage processValidationError(MethodArgumentNotValidException ex) {
    ...
    String errorMessage = "validationError" + this.processFieldErrors(result.getFieldErrors());
  ...
  private String processFieldErrors(List<FieldError> fieldErrors) {
    ...
    for (FieldError fieldError : fieldErrors) {
      ...
      sb.append("\n");
      sb.append(fieldError.getField()).append(": ").append(localizedErrorMessage);
    }

    String errorString = sb.toString();
    if (errorString.contains("exception") || errorString.contains("Exception")) {
