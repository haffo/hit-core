...
public class ContextBasedManagementController {
	...
	// File(System.getProperty("RESOURCE_BUNDLE_DIR"))
	// .getAbsolutePath() + "/cb";
	...
	public void publish(TestPlan testPlan) {
		...
		if (testCaseGroups != null) {
			for (TestCaseGroup testCaseGroup : testCaseGroups) {
				publish(testCaseGroup);
			}
		}
	}

	public void publish(TestCase testCase) {
		testCase.setScope(TestScope.GLOBAL);
	...
	public void publish(TestCaseGroup testCaseGroup) {
		testCaseGroup.setScope(TestScope.GLOBAL);
