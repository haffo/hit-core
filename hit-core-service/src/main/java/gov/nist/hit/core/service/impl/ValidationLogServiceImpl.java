...
@Service public class ValidationLogServiceImpl implements ValidationLogService {

    static final Logger logger = LoggerFactory.getLogger(ValidationLogServiceImpl.class);

    @Autowired private ValidationLogRepository validationLogRepository;
    ...
    @Autowired protected TestStepService testStepService;

    @Override public ValidationLog findOne(Long id) {
        // TODO Auto-generated method stub
        return validationLogRepository.findOne(id);
    }

    @Override public ValidationLog save(ValidationLog log) {
        // TODO Auto-generated method stub
        return validationLogRepository.saveAndFlush(log);
    }

    @Override public List<ValidationLog> findByUserId(Long userId) {
        // TODO Auto-generated method stub
        return validationLogRepository.findByUserId(userId);
    }

    @Override public List<ValidationLog> findByUserIdAndStage(Long userId, String testingStage) {
        // TODO Auto-generated method stub
        return validationLogRepository.findByUserIdAndStage(userId, testingStage);
    }

    @Override public List<ValidationLog> findByTestStepId(Long testStepId) {
        // TODO Auto-generated method stub
        return validationLogRepository.findByTestStepId(testStepId);
    }

    @Override public ValidationLog generateAndSave(Long userId, TestContext testContext,
                            ...
                            for (int i = 0; i < collection.size(); i++) {
                                ...
                                if (path != null && !"".equals(path)) {
                                    ...
                                    if (log.getErrorCountInSegment().containsKey(path)) {
                                        segmentErrorCount =
                                            log.getErrorCountInSegment().get(path) + 1;
                                    }
    ...
    @Override public List<ValidationLog> findAll() {
        // TODO Auto-generated method stub
        Date current = new Date();
        Date startDate = DateUtil.getFirstDateOfMonth(current);
        Date endDate = DateUtil.getLastDateOfMonth(current);
        return validationLogRepository.findAllBetweenDate(startDate, endDate);
    }
