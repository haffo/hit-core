...
public class ValidationLogUtil implements EnvironmentAware {
	...
	private static String DEFAULT_LOG_FORMAT = "%date [ToolScope -%toolScope] [Validation - %testingStage] %format - %messageId %validationResult [%errorCount errors %errorsInSegments, %warningCount warnings]";
	...
	public static String toString(ValidationLog validationLog) {
		...
		String logFormat = (environment == null || environment.getProperty("validation.logs.format") == null
				|| "".equals(environment.getProperty("validation.logs.format")) ? DEFAULT_LOG_FORMAT
						: environment.getProperty("validation.logs.format"));
		String logDateFormat = (environment == null || environment.getProperty("validation.logs.date.format") == null
				|| "".equals(environment.getProperty("validation.logs.date.format")) ? DEFAULT_LOG_DATE_FORMAT
						: environment.getProperty("validation.logs.date.format"));
		// Replace the fields in log
		if (logFormat.contains("%date")) {
			logFormat = logFormat.replace("%date", validationLog.getDate(logDateFormat));
		}
		if (logFormat.contains("%toolScope")) {
			logFormat = logFormat.replace("%toolScope", validationLog.getDomain());
		}
		...
		if (logFormat.contains("%errorsInSegments")) {
			...
			if (validationLog.getErrorCountInSegment().size() > 0) {
				...
				for (Map.Entry<String, Integer> segment : validationLog.getErrorCountInSegment().entrySet()) {
					if (!isFirst) {
						errorsInSegments.append(", ");
					}
					isFirst = false;
		...
		if (logFormat.contains("%warningCount")) {
			logFormat = logFormat.replace("%warningCount", String.valueOf(validationLog.getWarningCount()));
		}
