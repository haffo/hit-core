...
import gov.nist.healthcare.tools.core.services.util.HtmlUtils;
...
public abstract class ValidationReportGenerator {
	 ...
	 * @return
	 */
	public InputStream toPDF(String xml)
			...
			Transformer transformer = TransformerFactory.newInstance()
					.newTransformer(
							new StreamSource(new StringReader(
									getHtmlConversionXslt())));
			StreamSource source = new StreamSource(new StringReader(xml));
			ByteArrayOutputStream resultStream = new ByteArrayOutputStream();
			StreamResult result = new StreamResult(resultStream);
			transformer.transform(source, result);
			String htmlReport = HtmlUtils.repairStyle(new String(resultStream
					.toByteArray()));
	...
	public String toXHTML(String xml) throws ValidationReportException {
		try {
			...
			Transformer transformer = TransformerFactory.newInstance()
					.newTransformer(
							new StreamSource(new StringReader(
									getPdfConversionXslt())));
			StreamSource source = new StreamSource(new StringReader(
					bf.toString()));
			ByteArrayOutputStream resultStream = new ByteArrayOutputStream();
			StreamResult result = new StreamResult(resultStream);
			transformer.transform(source, result);
			String html = HtmlUtils.repairStyle(new String(resultStream
					.toByteArray()));
